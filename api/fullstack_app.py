...
# Try to mount the frontend build if it exists
frontend_dir = os.path.join(os.path.dirname(os.path.dirname(__file__)), "frontend", "out")
if os.path.exists(frontend_dir):
    app.mount("/", StaticFiles(directory=frontend_dir, html=True), name="frontend")
else:
    # If frontend build doesn't exist, mount at /app route instead of root
    # This allows the API to remain accessible at root path
    print("Frontend build not found. Serving API only.")

# For Vercel deployment, also make the app available as 'application'
application = app
